...
class wechatrobot
{
    ...
    {
        echo iconv("UTF-8","GB2312//IGNORE", "[①] 微信机器人正在启动,请稍后...\r\n");
        echo iconv("UTF-8","GB2312//IGNORE", "[②] 正在获取Appid...\r\n");
        $this->getAppid();
        echo iconv("UTF-8","GB2312//IGNORE", "[②] 正在获取UUID...\r\n");
        $this->getUUID();
        echo iconv("UTF-8","GB2312//IGNORE", "[④] 初始化获取Qrcode...\r\n");
        $this->getQrcode();
        echo iconv("UTF-8","GB2312//IGNORE", "好low,默认等待5秒扫描微信登陆二维码\r\n");
        echo iconv("UTF-8","GB2312//IGNORE", "[⑤] 初始化获取登陆Ticket...\r\n");
        sleep(15);
        $this->getTicket();
        echo iconv("UTF-8","GB2312//IGNORE", "[⑥] 正在登陆...\r\n");
        $this->webwxnewLoginPage();
        echo iconv("UTF-8","GB2312//IGNORE", "[⑦] 微信正在初始化...\r\n");
        $this->webwxInit();
        echo iconv("UTF-8","GB2312//IGNORE", "[⑧] 开启状态通知 ...\r\n");
        $this->webwxStatusNotify();
        echo iconv("UTF-8","GB2312//IGNORE", "[⑨] 正在获取用户列表...\r\n");
        $this->getMemberList();
        echo iconv("UTF-8","GB2312//IGNORE", "[⑩] 进入消息监听模式...\r\n");
        $this->listen();
    }
    ...
    public function listen()
    {
        ...
        while(true)
        {
            ...
            if($retcode == '1100')
            {
                echo iconv("UTF-8","GB2312//IGNORE", "从微信客户端上登出\r\n");
                break;
            }
            elseif($retcode == '1101')
            {
                echo iconv("UTF-8","GB2312//IGNORE", "从其它设备上登了网页微信\r\n");
                break;
            }
            else if($retcode == '0')
            {
                if($selector == '2')//有新消息
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "有新消息\r\n");
                    $syncRet = $this->webwxsync();
                    $this->handleMessage($syncRet);
                }
                elseif($selector == '3')//  # 未知
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "未知消息\r\n");
                    $syncRet = $this->webwxsync();
                    $this->handleMessage($syncRet); 
                }
                elseif($selector == '4')  # 通讯录更新
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "通讯录更新\r\n");
                    $syncRet = $this->webwxsync();
                    $this->handleMessage($syncRet);                    
                }
                elseif($selector == '6')# 可能是红包
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "可能是红包\r\n");
                    $syncRet = $this->webwxsync();
                    $this->handleMessage($syncRet);                      
                }
                elseif($selector == '7')
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "\t 线路异常...\r\n");
                    $this->synccheck();
                
                }
                elseif($selector == '0')
                {
                    echo iconv("UTF-8","GB2312//IGNORE", "未获取到任何信息...\r\n");
                    continue;
                }
            }
            usleep(800000);
    ...
    public function webwxgetbatchcontact($groupid)
    {
        ...
        if($response) {
            ...
            {
                echo iconv("UTF-8","GB2312//IGNORE", "获取组信息成功\r\n");
                ...
                echo iconv("UTF-8","GB2312//IGNORE", "获取组信息失败\r\n");
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '获取组信息请求无响应'));
        }            
    }
    ...
    public function sendTextMessage($messageText, $toUserName = 'filehelper')
    {
        ...
        if($response) {
            ...
            if(isset($response['BaseResponse']) && isset($response['BaseResponse']['Ret']) && $response['BaseResponse']['Ret'] == 0)
            {
                echo iconv("UTF-8","GB2312//IGNORE", "发送信息成功\r\n");
            } else {
                echo iconv("UTF-8","GB2312//IGNORE", "发送信息失败\r\n");
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '发送信息请求无响应'));
        }    
    }
    ...
    public function webwxsync()
    {
        ...
        if($response) {
            ...
            if(isset($response['BaseResponse']) && isset($response['BaseResponse']['Ret']) && $response['BaseResponse']['Ret'] == 0)
            {
                ...
                return $response;
                echo iconv("UTF-8","GB2312//IGNORE", "异步获取synckey成功\r\n");
            } else {
                echo iconv("UTF-8","GB2312//IGNORE", "异步获取synckey失败\r\n");
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '异步获取synckey请求无响应'));
        }    
    }
    ...
    public function synccheck()
    {
        ...
        if($response) {
            ...
            if(!empty($matchs)) {
                ...
                throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '异步检查失败'));
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '网络连接失败'));
        }   
    }            
    /**
    ...
    public function getAppId()
    {
        ...
        if(isset($matchs[1]) && !empty($matchs[1])) {
            ...
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '未获取到AppId'));
        }
    }
    ...
    public function getUUID()
    {
        ...
        if($response) {
            ...
            if($matchs[1] == '200' && !empty($matchs[2])) {
                ...
                throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '获取UUID 失败'));
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '网络连接失败'));
        }
        ...
        if($response) {
            ...
            if(isset($matchs)) {
                if(isset($matchs[1]) && $matchs[1] == '200') {
                    ...
                    throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '获取Ticket失败'));
                }
            } else {
                throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '匹配失败'));
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '网络连接失败'));
        }
    ...
    public function webwxnewLoginPage()
    {
        ...
        if($response) {
            ...
            if(!empty($matchs)) {
                ...
                throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '获取失败'));
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '登陆请求无响应'));
        }
    }
    ...
    public function webwxInit()
    {
        ...
        if($response) {
            if(isset($response['BaseResponse']) && isset($response['BaseResponse']['Ret']) && $response['BaseResponse']['Ret'] == 0) {
                echo iconv("UTF-8","GB2312//IGNORE", "微信初始化完成...\r\n");
                echo iconv("UTF-8","GB2312//IGNORE", "当前网页微信共{$response['Count']}聊天(包括个人、群、公众账号)...\r\n");
                echo iconv("UTF-8","GB2312//IGNORE", "当前网页微信聊天群名称...\r\n");
                ...
                if(!empty($contact_list)) {
                    ...
                    echo iconv("UTF-8","GB2312//IGNORE", "当前没有聊天记录\r\n");
                }
                $this->SyncKey = $response['SyncKey'];
                if(isset($response['SyncKey']['List']) && is_array($response['SyncKey']['List']))
                {
                ...
                throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '登陆初始化错误,可能是未正确扫描二维码'));
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '登陆初始化请求无响应'));
        }
    }
    ...
    public function webwxStatusNotify()
    {
        ...
        if($response) {
            ...
            if(isset($response['BaseResponse']) && isset($response['BaseResponse']['Ret']) && $response['BaseResponse']['Ret'] == 0)
            {
                echo iconv("UTF-8","GB2312//IGNORE", "开启状态通知成功\r\n");
            } else {
                echo iconv("UTF-8","GB2312//IGNORE", "开启状态通知失败\r\n");
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '开启状通知请求无响应'));
        }
    }
    /**
        ...
        if($response) {
            ...
            if(isset($response['BaseResponse']) && isset($response['BaseResponse']['Ret']) && $response['BaseResponse']['Ret'] == '0')
            {
                ...
                if(!empty($member_list)) {
                    ...
                    echo iconv("UTF-8","GB2312//IGNORE", '获取到当前用户列表,共'.count($this->member_list)."位...\r\n");
                } else {
                    echo iconv("UTF-8","GB2312//IGNORE", "当前没有用户列表信息\r\n");
                }
            } else {
                echo iconv("UTF-8","GB2312//IGNORE", "获取用户列表信息失败\r\n");
            }
        } else {
            throw new \Exception(iconv("UTF-8","GB2312//IGNORE", '获取用户列表信息请求无响应'));
        }
    }
